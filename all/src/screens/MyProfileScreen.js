...
export default function MyProfileScreen({ navigation }) {
  const { myProfile, interests = [], verificationRequests = [] } =
    useMatrimony();
  ...
  const theme = {
    bg: isDarkMode ? "#000000" : COLORS.bg,
    card: isDarkMode ? "#111111" : COLORS.white,
    text: isDarkMode ? "#FFFFFF" : COLORS.text,
    muted: isDarkMode ? "#D1D5DB" : COLORS.muted,
    border: isDarkMode ? "#333333" : COLORS.border,
    soft: isDarkMode ? "#1F2937" : COLORS.softOrange,
      ...
      >
        ...
        >
          <View style={styles.avatarBox}>
            <Image
              source={{
                uri: myProfile?.image || "https://i.pravatar.cc/600?img=8",
              }}
              style={styles.avatar}
            />
